...
public class Kasir {
    ...
    public void tampilkanDaftarMeja() {
        for (int i = 0; i < daftarMeja.length; i++) {
            if(daftarMeja[i].getPelanggan()!=null){
            System.out.println("MEJA " + i + " " +daftarMeja[i].getPelanggan().getNama());
        }else{
            System.out.println("MEJA " + i + " (KOSONG)");
        }
    }
    }
    ...
    public void tambahPelanggan(int nomorMeja, Pelanggan pelanggan) {
                ...
                boolean meja = daftarMeja[nomorMeja-1].isKosong();
                if (meja!=true){
    ...
    public void hapusPelanggan(int nomorMeja) {
         boolean meja = daftarMeja[nomorMeja-1].isKosong();
                if (meja!=true){
                    ...
                    System.out.println("Pelanggan pada meja " + nomorMeja + " sudah dihapus");
                }else{
    ...
    public void jalankan() {
        ...
        while (pilihan != 0) {
            ...
            switch (pilihan) {
                ...
                Boolean mejacek = daftarMeja[nomorMejaPesan - 1].isKosong();
                scanner.nextLine();  
                if (mejacek) {
                    tampilkanDaftarMenu();
                    while (!stopLoop) {
                ...
                System.out.print("Nomor meja : ");
                int mejacekmenu = scanner.nextInt();
                scanner.nextLine();
                boolean cekmeja = daftarMeja[mejacekmenu-1].isKosong();
                if(!cekmeja){
                    System.out.println("Meja "+ mejacekmenu + " tidak memiliki pesanan");
                }else{
                    int total = hitungHargaPesanan(mejacekmenu);
                    System.out.println("Harga pesanan di meja " + mejacekmenu + " adalah " + total);
                }break;
                case 6:
                 // untuk melihat pesanan pada meja tertentu
                // tampilkan pesan untuk memasukkan nomor meja 
                System.out.print("Nomor meja : ");
                int pesanmejaapa = scanner.nextInt();
                scanner.nextLine();
                tampilkanPesanan(pesanmejaapa);
                break;
